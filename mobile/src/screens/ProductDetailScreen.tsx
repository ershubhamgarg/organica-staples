...
import { getDiscountPercent, getDiscountedPrice, hasProductDiscount } from "@/lib/pricing";
...
export function ProductDetailScreen({ navigation, route }: Props) {
  ...
  const reviews = useProductStore((state) => state.reviewsByProduct[productId] || []);
  ...
  const submit = async () => {
    ...
    try {
      await submitReview(product.id, user?.id || null, name || "Anonymous", rating, comment.trim() || null);
      setRating(null);
      setComment("");
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Card style={styles.header}>
          ...
          <View style={styles.ratingRow}>
            {[0, 1, 2, 3, 4].map((star) => (
              <Star key={star} size={16} color={colors.gold} fill={star < Math.round(averageRating) ? colors.gold : "transparent"} />
            ))}
            <AppText variant="small">{averageRating.toFixed(1)} · {Math.max(reviews.length, product.review_count || 0)} reviews</AppText>
          </View>
        </Card>
        ...
        <Card style={styles.panel}>
          ...
          <View style={[styles.shipping, { borderColor: colors.border }]}>
            ...
            <AppText variant="small">Free shipping on orders over ₹500</AppText>
          </View>
        </Card>
        ...
        <Card style={styles.panel}>
          ...
          <View style={styles.nutritionGrid}>
            {NUTRITION_FACTS.map((fact) => (
              <View key={fact.label} style={[styles.nutrition, { borderColor: colors.border }]}>
          ...
          <View style={styles.ratingRow}>
            {[1, 2, 3, 4, 5].map((star) => (
              <Pressable key={star} onPress={() => setRating(rating === star ? null : star)}>
                <Star size={28} color={colors.gold} fill={star <= (rating || 0) ? colors.gold : "transparent"} />
              </Pressable>
            ))}
          </View>
          <AppInput placeholder="Your name" value={name} onChangeText={setName} />
          <AppInput placeholder="Your review" multiline value={comment} onChangeText={setComment} style={{ minHeight: 90 }} />
          <AppButton label="Submit feedback" onPress={submit} loading={isSubmitting} />
        </Card>
          ...
          <Card style={styles.panel}>
            ...
            {reviews.slice(0, 6).map((review) => (
              <View key={review.id} style={[styles.review, { borderColor: colors.border }]}>
                <AppText style={{ fontWeight: "800" }}>{review.user_name || "Anonymous"}</AppText>
                {review.comment && <AppText variant="small">"{review.comment}"</AppText>}
              </View>
            ))}
          </Card>
        )}

        <AppButton label="Go to cart" onPress={() => navigation.navigate("MainTabs")} />
      </ScrollView>
    </Screen>
  );
}

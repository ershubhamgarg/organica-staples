...
type Props = {
  navigation: {
    navigate: (screen: keyof RootStackParamList, params?: object) => void;
  };
};
...
export function HomeScreen({ navigation }: Props) {
  ...
  const categories = useMemo(() => {
    ...
    return [
      "All",
      ...Array.from(new Set(visible.map((product) => product.category))),
    ];
  }, [products]);

  const visibleProducts = useMemo(() => {
    let next = products.filter((p) => p.isVisible !== false);
    if (category !== "All") {
      ...
      >
        ...
        >
          ...
          <View style={styles.heroTop}>
            <AppText variant="eyebrow" style={{ color: "#F8F5F0" }}>
              Est. 2026
            </AppText>
        ...
        <View style={styles.sectionHeader}>
          ...
          >
            <AppText variant="small">
              {sort === "price-desc" ? "High to low" : "Low to high"}
            </AppText>
          </Pressable>
        </View>
            ...
            <Pressable
              ...
              style={[
                styles.chip,
                {
                  ...
                  backgroundColor:
                    item === category ? colors.primary : colors.surface,
                },
              ]}
            >
              <AppText
                variant="small"
                style={{
                  color: item === category ? "#fff" : colors.text,
                  fontWeight: "800",
                }}
              >
                {item}
        ...
        <Card style={styles.promise}>
          ...
          {["Fairly priced", "100% organic", "Ethically sourced"].map(
            (item) => (
              <View
                key={item}
                style={[styles.promiseRow, { borderColor: colors.border }]}
              >
        ...
        <Card style={styles.contact}>
          ...
          <AppInput
            ...
            onChangeText={(subject) => setContact({ ...contact, subject })}
          />
          <AppInput
            placeholder="Message"
            multiline
...
const styles = StyleSheet.create({
  ...
  heroOverlay: {
    ...
    backgroundColor: "rgba(20,22,17,0.58)",
  },
  heroTop: {
    flexDirection: "row",

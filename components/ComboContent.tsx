...
export default function ComboContent() {
  ...
  // Deep-link from a product card/page's "Add to Combo" CTA — a unit key to
  // pre-select on arrival. Read once per navigation (not synced via an
  // effect): once the customer edits anything themselves, `edits` below
  // takes over and this stops applying on its own, which is exactly the
  // one-shot behaviour a deep link should have.
  ...
  // The deep-linked item, folded into whatever's already in the cart — pure
  // derivation, so it naturally stops applying the instant `edits` exists
  // (the customer touched something themselves) without needing an effect
  // to "turn it off" again.
  const fromCartWithAddKey = useMemo(() => {
    if (!addKey || !items.some((item) => item.key === addKey)) return fromCart;
    if (fromCart[addKey] > 0) return fromCart;
    return { ...fromCart, [addKey]: 1 };
  }, [addKey, items, fromCart]);
  const picked = edits ?? fromCartWithAddKey;

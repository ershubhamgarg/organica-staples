-- Update the place_order_with_inventory function to fix the shipping CTC calculation
-- The issue was that shipping paid by the customer (e.g., ₹99) was not correctly offsetting 
-- the total freight cost in the Cost to Company (CTC) calculation, or was being miscalculated.
...
begin
  ...
  -- 3. CORE FINANCIAL LOGIC FIX
  -- We use the extra_shipping_amount directly as the net shipping cost to the company.
  -- Why? Because:
  --   Case 1 (Capped): Customer pays 99, Freight is 149. Extra is 50. Net loss = 50.
  --   Case 2 (Free): Customer pays 0, Freight is 149. Extra is 149. Net loss = 149.
  --   Case 3 (Under Cap): Customer pays 80, Freight is 80. Extra is 0. Net loss = 0.
  -- In all cases, extra_shipping_amount accurately represents the cash outflow 
  -- for shipping that was NOT covered by the customer.
  
  -- Cost to Company (CTC) = Wholesale + Net Shipping Loss + Packing + Gateway
  ...
  -- Profit/Loss = Total Amount - Cost to Company
  ...
  if p_launch_offer_email is not null then
    insert into public.launch_offer_claims (email, order_id)
    values (p_launch_offer_email, v_order.id)

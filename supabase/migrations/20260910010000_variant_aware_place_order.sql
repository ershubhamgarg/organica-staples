-- Makes place_order_with_inventory branch on an optional variant_id per
-- item. The non-variant branch (v_variant_id is null) is copied verbatim
-- from the current live version (20260602000000_fix_launch_offer_upsert.sql)
-- so existing checkout behavior is provably unchanged.
...
begin
  ...
  loop
    ...
    if v_variant_id is not null then
      select pvi.available_quantity, coalesce(pv.wholesale_price, p.wholesale_price, 0)
  ...
  loop
    ...
    if v_variant_id is not null then
      ...
      insert into public.order_items (order_id, product_id, variant_id, quantity, unit_price, wholesale_price)
      values (
        v_order.id,
        v_product_id,
        v_variant_id,
        ...
        (
          select coalesce(pv.wholesale_price, p.wholesale_price, 0)

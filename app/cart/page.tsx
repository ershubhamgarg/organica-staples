...
export default function CartPage() {
  ...
  if (!mounted) {
    return <div className="min-h-screen bg-stone-50 py-12 px-4 sm:px-6 lg:px-8"></div>;
  }
    ...
    <div className="min-h-screen bg-stone-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/#shop" className="inline-flex items-center gap-2 text-stone-500 hover:text-emerald-700 transition-colors text-sm font-medium">
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-12 text-center">
            ...
            <p className="text-stone-500 mb-8">Looks like you haven't added any organic staples yet.</p>
            <Link href="/#shop" className="inline-block bg-emerald-700 hover:bg-emerald-800 text-white font-medium px-8 py-3 rounded-xl transition-colors">
              Start Shopping
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
              {items.map((item) => (
                <div key={item.id} className="bg-white rounded-xl shadow-sm border border-stone-100 p-4 sm:p-6 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                  ...
                  <div className="flex-grow flex flex-col justify-between w-full">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="text-xs text-stone-500 mb-1 block">{item.category}</span>
                        <h3 className="text-lg font-medium text-stone-900 leading-tight">{item.name}</h3>
                        <p className="text-sm text-stone-500 mt-1">{item.weight}</p>
                      </div>
                      <button 
                ...
                <div className="border-t border-stone-100 pt-4 mb-6">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-2xl font-medium text-stone-900">
                      ₹{(totalPrice + (totalPrice > 0 && totalPrice <= 500 ? 50 : 0)).toFixed(2)}
                    </span>
                  </div>
                  <p className="text-xs text-stone-500 mt-1 text-right">Including taxes</p>
                </div>
